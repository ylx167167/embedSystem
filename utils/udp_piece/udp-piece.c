...
void udp_piece_deinit(udp_piece_t *udp_piece) {
  /* 释放资源 */
  if (udp_piece) {
    if (udp_piece->recv_buf) free(udp_piece->recv_buf);
...
uint8_t *udp_piece_get(udp_piece_t *udp_piece, int index, int *got_piece_size) {
  ...
  if (!udp_piece || !got_piece_size < 0) return NULL;
  ...
  *got_piece_size = 0;
  ...
  udp_piece->piece_buf[HEAD_POS_TOTAL_SIZE + 1] =
      (udp_piece->total_size & 0xff);
  // 所有分片的数量
  udp_piece->piece_buf[HEAD_POS_TOTAL_PIECES] = udp_piece->total_pieces >> 8;
  udp_piece->piece_buf[HEAD_POS_TOTAL_PIECES + 1] =
      (udp_piece->total_pieces & 0xff);
  //是否一张图片被区分为两次发送
  udp_piece->piece_buf[HEAD_POS_SUBPACKAGE_FLAG] = 0x02;      //不分包
  udp_piece->piece_buf[HEAD_POS_SUBPACKAGE_FLAG_END] = 0x01;  //默认结束
  // 分片编号，从0开始
  ...
  // 把用户数据拷贝到分片数据区
  memcpy(&udp_piece->piece_buf[HEAD_SIZE],
         &udp_piece->send_ptr[PIECE_FIX_SIZE * index], piece_size);
 ...
 * @return 返回指定分片编号的数据指针
 */
uint8_t *udp_piece_get_subpackage(udp_piece_t *udp_piece, int index,
                                  int *got_piece_size, int flag) {
  int piece_size = 0;
  ...
  *got_piece_size = 0;
  ...
  udp_piece->piece_buf[HEAD_POS_TOTAL_PIECES + 1] =
      (udp_piece->total_pieces & 0xff);
  //是否一张图片被区分为两次发送
  udp_piece->piece_buf[HEAD_POS_SUBPACKAGE_FLAG] = 0x01;  //确定分包
  if (flag == 0) {
    udp_piece->piece_buf[HEAD_POS_SUBPACKAGE_FLAG_END] = 0x01;  //分包结束
  } else {
    udp_piece->piece_buf[HEAD_POS_SUBPACKAGE_FLAG_END] = 0x02;  //分包未结束
  }
...
int udp_piece_merge(udp_piece_t *udp_piece, void *buf, int size) {
  ...
  for (int i = 0; i < size; i++) {
    if ((piece_buf[0] == 0xAF) && (piece_buf[1] == 0xAE)) break;
    piece_buf++;
    temp_size--;
  }
  ...
  while (temp_size > HEAD_SIZE) {
    // 当前分片的数据长度
    int data_len = (piece_buf[HEAD_POS_P_LENGTH] << 8) +
                   (piece_buf[HEAD_POS_P_LENGTH + 1]);
    if (temp_size >= (HEAD_SIZE + data_len)) {
      // 获取分片编号
      p_index = (piece_buf[HEAD_POS_P_INDEX] << 8) +
                (piece_buf[HEAD_POS_P_INDEX + 1]);
      if (udp_piece->total_size == 0)  // 重置后第一次收到数据
      {
        // 获取分片数据的总大小
        udp_piece->total_size = (piece_buf[HEAD_POS_TOTAL_SIZE] << 8) +
                                (piece_buf[HEAD_POS_TOTAL_SIZE + 1]);
        // 获取总的分片数量
        udp_piece->total_pieces = (piece_buf[HEAD_POS_TOTAL_PIECES] << 8) +
                                  (piece_buf[HEAD_POS_TOTAL_PIECES + 1]);
        udp_piece->recv_len = 0;
        udp_piece->recv_pieces = 0;
        if (udp_piece->recv_buf) free(udp_piece->recv_buf);
      ...
      UDP_DEBUG(
          "buf size: %d, piece_data_len: %d, p_index: %d, recv_pieces: %d, "
          "total_size: %d, total_pieces: %d\n",
          temp_size, data_len, p_index, udp_piece->recv_pieces,
          udp_piece->total_size, udp_piece->total_pieces);
      temp_total_size = (piece_buf[HEAD_POS_TOTAL_SIZE] << 8) +
                        (piece_buf[HEAD_POS_TOTAL_SIZE + 1]);
      temp_total_pieces = (piece_buf[HEAD_POS_TOTAL_PIECES] << 8) +
                          (piece_buf[HEAD_POS_TOTAL_PIECES + 1]);
      udp_piece->recv_pieces++;
      // 分析下新的分片是否和原来的分片组有区别
      if ((temp_total_size != udp_piece->total_size) ||
          (temp_total_pieces != udp_piece->total_pieces)) {
      ...
      piece_buf += HEAD_SIZE;
      temp_size -= HEAD_SIZE;
      memcpy(&udp_piece->recv_buf[PIECE_FIX_SIZE * p_index], piece_buf,
             data_len);
      piece_buf += data_len;
      temp_size -= data_len;
      ...
      if (udp_piece->recv_pieces == udp_piece->total_pieces) {
        ...
        if (udp_piece->recv_len == udp_piece->total_size) {
          ...
          UDP_ERR("recv_len != total_size! recv_len: %d, total_size: %d",
...
int udp_piece_merge_ex(udp_piece_t *udp_piece, void *buf, int size) {
  ...
  if (bytes_to_write != size) {
    UDP_ERR(
        "%s(%d) There is not enough space, only %d bytes, but need %d bytes\n",
  ...
  while (circular_buffer_size(udp_piece->circular_buffer) >= 2) {
    circular_buffer_get(udp_piece->circular_buffer, 0,
                        &value0); /* 通过索引获取当前值 */
    circular_buffer_get(udp_piece->circular_buffer, 1, &value1);
    if (value0 == 0xAF && value1 == 0xAE) {
      break;
    } else {
      circular_buffer_pop_front(udp_piece->circular_buffer,
                                1); /* 出队列一个元素 */
    }
  }
  ...
  while (circular_buffer_size(udp_piece->circular_buffer) > HEAD_SIZE) {
    /* 当前分片的数据长度（不含帧头） */
    circular_buffer_get(udp_piece->circular_buffer, HEAD_POS_P_LENGTH,
                        &value0); /* 通过索引获取当前值 */
    circular_buffer_get(udp_piece->circular_buffer, HEAD_POS_P_LENGTH + 1,
                        &value1);
    ...
    if (circular_buffer_size(udp_piece->circular_buffer) >=
        (HEAD_SIZE + data_len)) {
      circular_buffer_get(udp_piece->circular_buffer, HEAD_POS_P_INDEX,
                          &value0); /* 通过索引获取当前值 */
      circular_buffer_get(udp_piece->circular_buffer, HEAD_POS_P_INDEX + 1,
                          &value1);
      p_index = (value0 << 8) + value1;
      if (udp_piece->total_size == 0) /* 重置后第一次收到分片 */
      {
        /* 计算当前分片所属分片组数据的总大小 */
        circular_buffer_get(udp_piece->circular_buffer, HEAD_POS_TOTAL_SIZE,
                            &value0); /* 通过索引获取当前值 */
        circular_buffer_get(udp_piece->circular_buffer, HEAD_POS_TOTAL_SIZE + 1,
                            &value1);
        udp_piece->total_size = (value0 << 8) + value1;

        /* 计算当前分片所属分片组的总分片数量 */
        circular_buffer_get(udp_piece->circular_buffer, HEAD_POS_TOTAL_PIECES,
                            &value0); /* 通过索引获取当前值 */
        circular_buffer_get(udp_piece->circular_buffer,
                            HEAD_POS_TOTAL_PIECES + 1, &value1);
      ...
      UDP_DEBUG(
          "buf size: %d, piece_data_len: %d, p_index: %d, recv_pieces: %d, "
          "total_size: %d, total_pieces: %d",
          circular_buffer_size(udp_piece->circular_buffer), data_len, p_index,
          udp_piece->recv_pieces, udp_piece->total_size,
      ...
      circular_buffer_get(udp_piece->circular_buffer, HEAD_POS_TOTAL_PIECES,
                          &value0); /* 通过索引获取当前值 */
      circular_buffer_get(udp_piece->circular_buffer, HEAD_POS_TOTAL_PIECES + 1,
                          &value1);
      tmp_total_pieces = (value0 << 8) + value1;
      udp_piece->recv_pieces++;
      if ((tmp_total_pieces != udp_piece->total_pieces) ||
          (tmp_total_size != udp_piece->total_size)) {
        UDP_DEBUG(
            "Discard current frame, total_pieces:%d, cur_total_pieces:%d, "
            "recv_pieces:%d, total_size:%d, cur_total_size:%d\n",
            udp_piece->total_pieces, tmp_total_pieces, udp_piece->recv_pieces,
      ...
      // 读取分片数据
      bytes_to_read = circular_buffer_read(
          udp_piece->circular_buffer,
          &udp_piece->recv_buf[PIECE_FIX_SIZE * p_index], data_len);
      if (bytes_to_read != data_len) {
        UDP_ERR(
            "%s(%d) There is not enough space, only %d bytes, but need %d "
            "bytes\n",
            __FUNCTION__, __LINE__, bytes_to_write, size);
        return (-1);
      }
      ...
      if (udp_piece->recv_pieces == udp_piece->total_pieces) {
        ...
        if (udp_piece->recv_len ==
            udp_piece->total_size) /* current frame is received completely, call
                                      Process() */
        {
          /* 组成了一帧数据 */
          get_all_pieces = 1;
