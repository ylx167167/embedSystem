...
#define HEAD_POS_TOTAL_SIZE \
  2  // 所有分片数据的大小（不包括HEAD） //total data length of all pieces
#define HEAD_POS_TOTAL_PIECES 4     // 所有分片的数量
#define HEAD_POS_SUBPACKAGE_FLAG 6  //是否一张图的大小被区分为两张
#define HEAD_POS_SUBPACKAGE_FLAG_END 7  //分包结束
#define HEAD_POS_P_INDEX 8              // 分片序号，从0开始
#define HEAD_POS_P_LENGTH 10            // 当前分片数据的大小
#define HEAD_SIZE 14                    // 头部大小
...
typedef struct udp_piece {
  ...
  uint8_t recv_sub_package;  //一帧被分为多个包发送 0x01是分包 0x02是不分包
  uint8_t recv_sub_package_end;  //如果分包结束则 0x01 如果未结束则0x02
