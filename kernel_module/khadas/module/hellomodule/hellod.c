...
static void meson_i2c_set_clk_div_std(struct meson_i2c *i2c) {
  ...
  dev_dbg(i2c->dev, "%s: clk %lu, freq %u, div_h %u, div_l %u\n", __func__,
          clk_rate, i2c->frequency, div_h, div_l);
}
...
//   for (i = 0; i < min_t(int, 4, len); i++) *buf++ = (rdata0 >> i * 8) & 0xff;
...
static void meson_i2c_put_data(struct meson_i2c *i2c, char *buf, int len) {
  ...
  for (i = 0; i < min_t(int, 4, len); i++) wdata0 |= *buf++ << (i * 8);

  for (i = 4; i < min_t(int, 8, len); i++) wdata1 |= *buf++ << ((i - 4) * 8);

  writel(wdata0, i2c->regs + REG_TOK_WDATA0);
  writel(wdata1, i2c->regs + REG_TOK_WDATA1);

  dev_dbg(i2c->dev, "%s: data %08x %08x len %d\n", __func__, wdata0, wdata1,
          len);
}
...
static void meson_i2c_prepare_xfer(struct meson_i2c *i2c) {
  ...
  for (i = 0; i < i2c->count - 1; i++) meson_i2c_add_token(i2c, TOKEN_DATA);
  ...
  if (write) meson_i2c_put_data(i2c, i2c->msg->buf + i2c->pos, i2c->count);
}
...
static void meson_i2c_do_start(struct meson_i2c *i2c, struct i2c_msg *msg) {
  ...
  token =
      (msg->flags & I2C_M_RD) ? TOKEN_SLAVE_ADDR_READ : TOKEN_SLAVE_ADDR_WRITE;
...
static int meson_i2c_xfer_msg(struct meson_i2c *i2c, struct i2c_msg *msg,
  ...
  if (!(msg->flags & I2C_M_NOSTART)) meson_i2c_do_start(i2c, msg);
...
static ssize_t meson_i2c_speed_show(struct device *child,
  ...
  struct meson_i2c *i2c = (struct meson_i2c *)dev_get_drvdata(child);
...
static DEVICE_ATTR(speed, 0644, meson_i2c_speed_show, meson_i2c_speed_store);
...
static int meson_i2c_probe(struct platform_device *pdev) {
  ...
  ret = devm_request_irq(&pdev->dev, i2c->irq, meson_i2c_irq, 0,
  ...
  strlcpy(i2c->adap.name, "Meson I2C adapter", sizeof(i2c->adap.name));
  ...
  if (ret) dev_err(&pdev->dev, "Creat speed debug sysfs failed\n");
